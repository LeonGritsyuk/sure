        ...
        result = scope
                  .select(
                    ActiveRecord::Base.sanitize_sql_array([
                      "COALESCE(SUM(CASE WHEN entries.amount >= 0 AND transactions.kind NOT IN (?) THEN ABS(entries.amount * converted.rate) ELSE 0 END), 0) as expense_total",
                      Transaction::TRANSFER_KINDS
                    ]),
                    ActiveRecord::Base.sanitize_sql_array([
                      "COALESCE(SUM(CASE WHEN entries.amount < 0 AND transactions.kind NOT IN (?) THEN ABS(entries.amount * converted.rate) ELSE 0 END), 0) as income_total",
                      Transaction::TRANSFER_KINDS
                    ]),
                    ActiveRecord::Base.sanitize_sql_array([
                      "COALESCE(SUM(CASE WHEN entries.amount < 0 AND transactions.kind IN (?) THEN ABS(entries.amount * converted.rate) ELSE 0 END), 0) as transfer_inflow_total",
                      Transaction::TRANSFER_KINDS
                    ]),
                    ActiveRecord::Base.sanitize_sql_array([
                      "COALESCE(SUM(CASE WHEN entries.amount >= 0 AND transactions.kind IN (?) THEN ABS(entries.amount * converted.rate) ELSE 0 END), 0) as transfer_outflow_total",
                      Transaction::TRANSFER_KINDS
                    ]),
                  ...
                  .joins(
                    ActiveRecord::Base.sanitize_sql_array([
                      "CROSS JOIN LATERAL (SELECT CASE WHEN entries.currency = ? THEN 1 ELSE er.rate END AS rate) converted",
                      family.currency
                    ])
                  )
                  .take
